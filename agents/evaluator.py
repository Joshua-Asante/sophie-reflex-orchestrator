...
"""Evaluator Agent using OpenRouter for all LLM calls."""
from .base_agent import BaseAgent, AgentConfig, AgentResult, AgentStatus, CircuitState

logger = structlog.get_logger()

# Modularization imports (scaffold)
# TODO: gradually move prompts/parsers/scoring logic into these modules
try:
    from agents.evaluator.prompts import build_category_prompt, build_overall_feedback_prompt  # type: ignore
    from agents.evaluator.parsers import parse_json_response  # type: ignore
    from agents.evaluator.scoring import EvaluationStats, update_stats  # type: ignore
except Exception:
    pass
...
class EvaluatorAgent(BaseAgent):
    ...
    async def _evaluate_category_enhanced(self, category_name: str, task: str, solution: str,
        ...
        # Generate enhanced evaluation prompt (prefer modular builder)
        try:
            prompt = build_category_prompt(category_name, task, solution, {
        ...
        # Parse the enhanced response (JSON-first)
        content = llm_response.get("content", "")
        try:
            parsed = parse_json_response(content)  # type: ignore[name-defined]
        except Exception:
            parsed = None
        if parsed is None:
            parsed_result = self._parse_enhanced_evaluation_response(content, levels, sub_criteria)
        else:
            try:
...
"""
    ...
    async def _call_openrouter(self, prompt: str, context: Dict[str, Any]) -> Dict[str, Any]:
        """Use OpenRouter chat completions; prefer JSON output when requested."""
        ...
        try:
            ...
            return {
                ...
                "reasoning": "Evaluation performed via OpenRouter",
